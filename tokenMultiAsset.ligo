...
// Token Multi-Asset type
#include "../partials/contractTypes/tokenMultiAssetTypes.ligo"
...
(* total_supply View *)
[@view] function total_supply(const token_id : tokenIdNatType; const s : tokenMultiAssetStorageType) : tokenBalanceNatType is
...
block {
    ...
    for mintParams in list mintListParams block {
        ...
        // Update token total supply - if none, set to new amount minted
        s.total_supply[tokenId] := case s.total_supply[tokenId] of [
                Some(_balance) -> _balance + amount
            |   None           -> amount
        ];
    }
...
block {
    ...
    for burnParams in list burnListParams block {
        ...
        s.total_supply[tokenId] := case s.total_supply[tokenId] of [
                Some(_balance) -> abs(_balance - amount)
            |   None           -> 0n
        ];
    }
